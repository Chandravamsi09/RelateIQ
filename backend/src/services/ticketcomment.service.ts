/**
 * RelateIQ Enterprise Domain Service: TicketCommentService
 * Domain: support
 * Encapsulates multi-tenant transaction orchestration, validation, event dispatching, and audit persistence.
 */

import { TicketCommentAggregate, TicketCommentProps } from '../domain/aggregates/ticketcomment.aggregate';
...
export class TicketCommentService {
  private inMemoryStore: Map<string, TicketCommentAggregate> = new Map();

  constructor() {
    const seed = TicketCommentAggregate.create({
      id: 'ticketcomment-seed-01',
      tenantId: 'tenant-acme-corp',
      name: 'Default Enterprise TicketComment',
      title: 'Enterprise TicketComment Seed',
      status: 'ACTIVE',
  ...
  public async getById(tenantId: string, id: string): Promise<Record<string, any> | null> {
    ...
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation boundary violated for TicketComment');
    }
  ...
  public async create(tenantId: string, actor: string, dto: CreateTicketCommentDTO): Promise<Record<string, any>> {
    const id = 'ticketcomment-' + Date.now() + '-' + Math.random().toString(36).substr(2, 6);
    const aggregate = TicketCommentAggregate.create({
      id,
      tenantId,
      name: dto.name || dto.title || 'TicketComment Item',
  ...
  public async update(tenantId: string, id: string, actor: string, dto: UpdateTicketCommentDTO): Promise<Record<string, any>> {
    const aggregate = this.inMemoryStore.get(id);
    if (!aggregate) {
      throw new Error('TicketComment with ID ' + id + ' not found');
    }
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation violation on TicketComment update');
    }
