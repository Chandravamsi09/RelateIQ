/**
 * RelateIQ Enterprise Domain Service: UserPreferenceService
 * Domain: settings
 * Encapsulates multi-tenant transaction orchestration, validation, event dispatching, and audit persistence.
 */

import { UserPreferenceAggregate, UserPreferenceProps } from '../domain/aggregates/userpreference.aggregate';
...
export class UserPreferenceService {
  private inMemoryStore: Map<string, UserPreferenceAggregate> = new Map();

  constructor() {
    const seed = UserPreferenceAggregate.create({
      id: 'userpreference-seed-01',
      tenantId: 'tenant-acme-corp',
      name: 'Default Enterprise UserPreference',
      title: 'Enterprise UserPreference Seed',
      status: 'ACTIVE',
  ...
  public async getById(tenantId: string, id: string): Promise<Record<string, any> | null> {
    ...
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation boundary violated for UserPreference');
    }
  ...
  public async create(tenantId: string, actor: string, dto: CreateUserPreferenceDTO): Promise<Record<string, any>> {
    const id = 'userpreference-' + Date.now() + '-' + Math.random().toString(36).substr(2, 6);
    const aggregate = UserPreferenceAggregate.create({
      id,
      tenantId,
      name: dto.name || dto.title || 'UserPreference Item',
  ...
  public async update(tenantId: string, id: string, actor: string, dto: UpdateUserPreferenceDTO): Promise<Record<string, any>> {
    const aggregate = this.inMemoryStore.get(id);
    if (!aggregate) {
      throw new Error('UserPreference with ID ' + id + ' not found');
    }
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation violation on UserPreference update');
    }
