/**
 * RelateIQ Enterprise Domain Service: TaskService
 * Domain: omnichannel
 * Encapsulates multi-tenant transaction orchestration, validation, event dispatching, and audit persistence.
 */

import { TaskAggregate, TaskProps } from '../domain/aggregates/task.aggregate';
...
export class TaskService {
  private inMemoryStore: Map<string, TaskAggregate> = new Map();

  constructor() {
    const seed = TaskAggregate.create({
      id: 'task-seed-01',
      tenantId: 'tenant-acme-corp',
      name: 'Default Enterprise Task',
      title: 'Enterprise Task Seed',
      status: 'ACTIVE',
  ...
  public async getById(tenantId: string, id: string): Promise<Record<string, any> | null> {
    ...
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation boundary violated for Task');
    }
  ...
  public async create(tenantId: string, actor: string, dto: CreateTaskDTO): Promise<Record<string, any>> {
    const id = 'task-' + Date.now() + '-' + Math.random().toString(36).substr(2, 6);
    const aggregate = TaskAggregate.create({
      id,
      tenantId,
      name: dto.name || dto.title || 'Task Item',
  ...
  public async update(tenantId: string, id: string, actor: string, dto: UpdateTaskDTO): Promise<Record<string, any>> {
    const aggregate = this.inMemoryStore.get(id);
    if (!aggregate) {
      throw new Error('Task with ID ' + id + ' not found');
    }
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation violation on Task update');
    }
