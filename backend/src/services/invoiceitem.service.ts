/**
 * RelateIQ Enterprise Domain Service: InvoiceItemService
 * Domain: billing
 * Encapsulates multi-tenant transaction orchestration, validation, event dispatching, and audit persistence.
 */

import { InvoiceItemAggregate, InvoiceItemProps } from '../domain/aggregates/invoiceitem.aggregate';
...
export class InvoiceItemService {
  private inMemoryStore: Map<string, InvoiceItemAggregate> = new Map();

  constructor() {
    const seed = InvoiceItemAggregate.create({
      id: 'invoiceitem-seed-01',
      tenantId: 'tenant-acme-corp',
      name: 'Default Enterprise InvoiceItem',
      title: 'Enterprise InvoiceItem Seed',
      status: 'ACTIVE',
  ...
  public async getById(tenantId: string, id: string): Promise<Record<string, any> | null> {
    ...
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation boundary violated for InvoiceItem');
    }
  ...
  public async create(tenantId: string, actor: string, dto: CreateInvoiceItemDTO): Promise<Record<string, any>> {
    const id = 'invoiceitem-' + Date.now() + '-' + Math.random().toString(36).substr(2, 6);
    const aggregate = InvoiceItemAggregate.create({
      id,
      tenantId,
      name: dto.name || dto.title || 'InvoiceItem Item',
  ...
  public async update(tenantId: string, id: string, actor: string, dto: UpdateInvoiceItemDTO): Promise<Record<string, any>> {
    const aggregate = this.inMemoryStore.get(id);
    if (!aggregate) {
      throw new Error('InvoiceItem with ID ' + id + ' not found');
    }
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation violation on InvoiceItem update');
    }
