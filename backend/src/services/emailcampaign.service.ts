/**
 * RelateIQ Enterprise Domain Service: EmailCampaignService
 * Domain: marketing
 * Encapsulates multi-tenant transaction orchestration, validation, event dispatching, and audit persistence.
 */

import { EmailCampaignAggregate, EmailCampaignProps } from '../domain/aggregates/emailcampaign.aggregate';
...
export class EmailCampaignService {
  private inMemoryStore: Map<string, EmailCampaignAggregate> = new Map();

  constructor() {
    const seed = EmailCampaignAggregate.create({
      id: 'emailcampaign-seed-01',
      tenantId: 'tenant-acme-corp',
      name: 'Default Enterprise EmailCampaign',
      title: 'Enterprise EmailCampaign Seed',
      status: 'ACTIVE',
  ...
  public async getById(tenantId: string, id: string): Promise<Record<string, any> | null> {
    ...
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation boundary violated for EmailCampaign');
    }
  ...
  public async create(tenantId: string, actor: string, dto: CreateEmailCampaignDTO): Promise<Record<string, any>> {
    const id = 'emailcampaign-' + Date.now() + '-' + Math.random().toString(36).substr(2, 6);
    const aggregate = EmailCampaignAggregate.create({
      id,
      tenantId,
      name: dto.name || dto.title || 'EmailCampaign Item',
  ...
  public async update(tenantId: string, id: string, actor: string, dto: UpdateEmailCampaignDTO): Promise<Record<string, any>> {
    const aggregate = this.inMemoryStore.get(id);
    if (!aggregate) {
      throw new Error('EmailCampaign with ID ' + id + ' not found');
    }
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation violation on EmailCampaign update');
    }
