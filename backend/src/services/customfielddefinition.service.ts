/**
 * RelateIQ Enterprise Domain Service: CustomFieldDefinitionService
 * Domain: customization
 * Encapsulates multi-tenant transaction orchestration, validation, event dispatching, and audit persistence.
 */

import { CustomFieldDefinitionAggregate, CustomFieldDefinitionProps } from '../domain/aggregates/customfielddefinition.aggregate';
...
export class CustomFieldDefinitionService {
  private inMemoryStore: Map<string, CustomFieldDefinitionAggregate> = new Map();

  constructor() {
    const seed = CustomFieldDefinitionAggregate.create({
      id: 'customfielddefinition-seed-01',
      tenantId: 'tenant-acme-corp',
      name: 'Default Enterprise CustomFieldDefinition',
      title: 'Enterprise CustomFieldDefinition Seed',
      status: 'ACTIVE',
  ...
  public async getById(tenantId: string, id: string): Promise<Record<string, any> | null> {
    ...
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation boundary violated for CustomFieldDefinition');
    }
  ...
  public async create(tenantId: string, actor: string, dto: CreateCustomFieldDefinitionDTO): Promise<Record<string, any>> {
    const id = 'customfielddefinition-' + Date.now() + '-' + Math.random().toString(36).substr(2, 6);
    const aggregate = CustomFieldDefinitionAggregate.create({
      id,
      tenantId,
      name: dto.name || dto.title || 'CustomFieldDefinition Item',
  ...
  public async update(tenantId: string, id: string, actor: string, dto: UpdateCustomFieldDefinitionDTO): Promise<Record<string, any>> {
    const aggregate = this.inMemoryStore.get(id);
    if (!aggregate) {
      throw new Error('CustomFieldDefinition with ID ' + id + ' not found');
    }
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation violation on CustomFieldDefinition update');
    }
