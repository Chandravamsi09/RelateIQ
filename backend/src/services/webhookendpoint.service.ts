/**
 * RelateIQ Enterprise Domain Service: WebhookEndpointService
 * Domain: integrations
 * Encapsulates multi-tenant transaction orchestration, validation, event dispatching, and audit persistence.
 */

import { WebhookEndpointAggregate, WebhookEndpointProps } from '../domain/aggregates/webhookendpoint.aggregate';
...
export class WebhookEndpointService {
  private inMemoryStore: Map<string, WebhookEndpointAggregate> = new Map();

  constructor() {
    const seed = WebhookEndpointAggregate.create({
      id: 'webhookendpoint-seed-01',
      tenantId: 'tenant-acme-corp',
      name: 'Default Enterprise WebhookEndpoint',
      title: 'Enterprise WebhookEndpoint Seed',
      status: 'ACTIVE',
  ...
  public async getById(tenantId: string, id: string): Promise<Record<string, any> | null> {
    ...
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation boundary violated for WebhookEndpoint');
    }
  ...
  public async create(tenantId: string, actor: string, dto: CreateWebhookEndpointDTO): Promise<Record<string, any>> {
    const id = 'webhookendpoint-' + Date.now() + '-' + Math.random().toString(36).substr(2, 6);
    const aggregate = WebhookEndpointAggregate.create({
      id,
      tenantId,
      name: dto.name || dto.title || 'WebhookEndpoint Item',
  ...
  public async update(tenantId: string, id: string, actor: string, dto: UpdateWebhookEndpointDTO): Promise<Record<string, any>> {
    const aggregate = this.inMemoryStore.get(id);
    if (!aggregate) {
      throw new Error('WebhookEndpoint with ID ' + id + ' not found');
    }
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation violation on WebhookEndpoint update');
    }
