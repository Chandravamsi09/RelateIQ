/**
 * RelateIQ Enterprise Domain Service: ActivityService
 * Domain: omnichannel
 * Encapsulates multi-tenant transaction orchestration, validation, event dispatching, and audit persistence.
 */

import { ActivityAggregate, ActivityProps } from '../domain/aggregates/activity.aggregate';
...
export class ActivityService {
  private inMemoryStore: Map<string, ActivityAggregate> = new Map();

  constructor() {
    const seed = ActivityAggregate.create({
      id: 'activity-seed-01',
      tenantId: 'tenant-acme-corp',
      name: 'Default Enterprise Activity',
      title: 'Enterprise Activity Seed',
      status: 'ACTIVE',
  ...
  public async getById(tenantId: string, id: string): Promise<Record<string, any> | null> {
    ...
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation boundary violated for Activity');
    }
  ...
  public async create(tenantId: string, actor: string, dto: CreateActivityDTO): Promise<Record<string, any>> {
    const id = 'activity-' + Date.now() + '-' + Math.random().toString(36).substr(2, 6);
    const aggregate = ActivityAggregate.create({
      id,
      tenantId,
      name: dto.name || dto.title || 'Activity Item',
  ...
  public async update(tenantId: string, id: string, actor: string, dto: UpdateActivityDTO): Promise<Record<string, any>> {
    const aggregate = this.inMemoryStore.get(id);
    if (!aggregate) {
      throw new Error('Activity with ID ' + id + ' not found');
    }
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation violation on Activity update');
    }
