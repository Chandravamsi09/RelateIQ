/**
 * RelateIQ Enterprise Domain Service: UserService
 * Domain: identity
...
export class UserService {
  private inMemoryStore: Map<string, UserAggregate> = new Map();

  constructor() {
    const seed = UserAggregate.create({
      id: 'user-seed-01',
      tenantId: 'tenant-acme-corp',
      name: 'Default Enterprise User',
      title: 'Enterprise User Seed',
      status: 'ACTIVE',
  ...
  public async getById(tenantId: string, id: string): Promise<Record<string, any> | null> {
    ...
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation boundary violated for User');
    }
  ...
  public async create(tenantId: string, actor: string, dto: CreateUserDTO): Promise<Record<string, any>> {
    const id = 'user-' + Date.now() + '-' + Math.random().toString(36).substr(2, 6);
    const aggregate = UserAggregate.create({
      id,
      tenantId,
      name: dto.name || dto.title || 'User Item',
  ...
  public async update(tenantId: string, id: string, actor: string, dto: UpdateUserDTO): Promise<Record<string, any>> {
    const aggregate = this.inMemoryStore.get(id);
    if (!aggregate) {
      throw new Error('User with ID ' + id + ' not found');
    }
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation violation on User update');
    }
