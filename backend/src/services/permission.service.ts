/**
 * RelateIQ Enterprise Domain Service: PermissionService
 * Domain: identity
 * Encapsulates multi-tenant transaction orchestration, validation, event dispatching, and audit persistence.
 */

import { PermissionAggregate, PermissionProps } from '../domain/aggregates/permission.aggregate';
...
export class PermissionService {
  private inMemoryStore: Map<string, PermissionAggregate> = new Map();

  constructor() {
    const seed = PermissionAggregate.create({
      id: 'permission-seed-01',
      tenantId: 'tenant-acme-corp',
      name: 'Default Enterprise Permission',
      title: 'Enterprise Permission Seed',
      status: 'ACTIVE',
  ...
  public async getById(tenantId: string, id: string): Promise<Record<string, any> | null> {
    ...
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation boundary violated for Permission');
    }
  ...
  public async create(tenantId: string, actor: string, dto: CreatePermissionDTO): Promise<Record<string, any>> {
    const id = 'permission-' + Date.now() + '-' + Math.random().toString(36).substr(2, 6);
    const aggregate = PermissionAggregate.create({
      id,
      tenantId,
      name: dto.name || dto.title || 'Permission Item',
  ...
  public async update(tenantId: string, id: string, actor: string, dto: UpdatePermissionDTO): Promise<Record<string, any>> {
    const aggregate = this.inMemoryStore.get(id);
    if (!aggregate) {
      throw new Error('Permission with ID ' + id + ' not found');
    }
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation violation on Permission update');
    }
