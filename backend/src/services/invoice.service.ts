/**
 * RelateIQ Enterprise Domain Service: InvoiceService
 * Domain: billing
 * Encapsulates multi-tenant transaction orchestration, validation, event dispatching, and audit persistence.
 */

import { InvoiceAggregate, InvoiceProps } from '../domain/aggregates/invoice.aggregate';
...
export class InvoiceService {
  private inMemoryStore: Map<string, InvoiceAggregate> = new Map();

  constructor() {
    const seed = InvoiceAggregate.create({
      id: 'invoice-seed-01',
      tenantId: 'tenant-acme-corp',
      name: 'Default Enterprise Invoice',
      title: 'Enterprise Invoice Seed',
      status: 'ACTIVE',
  ...
  public async getById(tenantId: string, id: string): Promise<Record<string, any> | null> {
    ...
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation boundary violated for Invoice');
    }
  ...
  public async create(tenantId: string, actor: string, dto: CreateInvoiceDTO): Promise<Record<string, any>> {
    const id = 'invoice-' + Date.now() + '-' + Math.random().toString(36).substr(2, 6);
    const aggregate = InvoiceAggregate.create({
      id,
      tenantId,
      name: dto.name || dto.title || 'Invoice Item',
  ...
  public async update(tenantId: string, id: string, actor: string, dto: UpdateInvoiceDTO): Promise<Record<string, any>> {
    const aggregate = this.inMemoryStore.get(id);
    if (!aggregate) {
      throw new Error('Invoice with ID ' + id + ' not found');
    }
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation violation on Invoice update');
    }
