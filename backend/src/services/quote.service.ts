/**
 * RelateIQ Enterprise Domain Service: QuoteService
 * Domain: cpq
 * Encapsulates multi-tenant transaction orchestration, validation, event dispatching, and audit persistence.
 */

import { QuoteAggregate, QuoteProps } from '../domain/aggregates/quote.aggregate';
...
export class QuoteService {
  private inMemoryStore: Map<string, QuoteAggregate> = new Map();

  constructor() {
    const seed = QuoteAggregate.create({
      id: 'quote-seed-01',
      tenantId: 'tenant-acme-corp',
      name: 'Default Enterprise Quote',
      title: 'Enterprise Quote Seed',
      status: 'ACTIVE',
  ...
  public async getById(tenantId: string, id: string): Promise<Record<string, any> | null> {
    ...
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation boundary violated for Quote');
    }
  ...
  public async create(tenantId: string, actor: string, dto: CreateQuoteDTO): Promise<Record<string, any>> {
    const id = 'quote-' + Date.now() + '-' + Math.random().toString(36).substr(2, 6);
    const aggregate = QuoteAggregate.create({
      id,
      tenantId,
      name: dto.name || dto.title || 'Quote Item',
  ...
  public async update(tenantId: string, id: string, actor: string, dto: UpdateQuoteDTO): Promise<Record<string, any>> {
    const aggregate = this.inMemoryStore.get(id);
    if (!aggregate) {
      throw new Error('Quote with ID ' + id + ' not found');
    }
    if (aggregate.getTenantId() !== tenantId) {
      throw new Error('Tenant isolation violation on Quote update');
    }
