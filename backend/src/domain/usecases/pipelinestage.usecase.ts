/**
 * RelateIQ Domain Use Case: ProcessPipelineStageUseCase
...
export interface ProcessPipelineStageCommand {
  tenantId: string;
  actor: string;
...
export class ProcessPipelineStageUseCase {
  ...
  public async execute(command: ProcessPipelineStageCommand): Promise<{ success: boolean; result: any; auditEventId: string }> {
    if (!command.tenantId) {
