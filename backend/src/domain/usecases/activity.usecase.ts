/**
 * RelateIQ Domain Use Case: ProcessActivityUseCase
...
export class ProcessActivityUseCase {
  ...
  public async execute(command: ProcessActivityCommand): Promise<{ success: boolean; result: any; auditEventId: string }> {
    if (!command.tenantId) {
