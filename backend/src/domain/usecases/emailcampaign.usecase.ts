/**
 * RelateIQ Domain Use Case: ProcessEmailCampaignUseCase
 * Implements business rule enforcement, event publication, and cross-aggregate orchestration.
 */

import { EmailCampaignRepository } from '../repositories/emailcampaign.repository';

export interface ProcessEmailCampaignCommand {
  tenantId: string;
  actor: string;
...
export class ProcessEmailCampaignUseCase {
  ...
  public async execute(command: ProcessEmailCampaignCommand): Promise<{ success: boolean; result: any; auditEventId: string }> {
    if (!command.tenantId) {
