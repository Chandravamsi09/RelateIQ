/**
 * RelateIQ Domain Use Case: ProcessTerritoryUseCase
...
export interface ProcessTerritoryCommand {
  tenantId: string;
  actor: string;
...
export class ProcessTerritoryUseCase {
  ...
  public async execute(command: ProcessTerritoryCommand): Promise<{ success: boolean; result: any; auditEventId: string }> {
    if (!command.tenantId) {
