/**
 * RelateIQ Domain Use Case: ProcessWebhookEndpointUseCase
...
export interface ProcessWebhookEndpointCommand {
  tenantId: string;
  actor: string;
...
export class ProcessWebhookEndpointUseCase {
  private repository: WebhookEndpointRepository;

  constructor() {
    this.repository = new WebhookEndpointRepository();
  }

  public async execute(command: ProcessWebhookEndpointCommand): Promise<{ success: boolean; result: any; auditEventId: string }> {
    if (!command.tenantId) {
