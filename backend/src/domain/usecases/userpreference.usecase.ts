...
export class ProcessUserPreferenceUseCase {
  private repository: UserPreferenceRepository;

  constructor() {
    this.repository = new UserPreferenceRepository();
  }

  public async execute(command: ProcessUserPreferenceCommand): Promise<{ success: boolean; result: any; auditEventId: string }> {
    if (!command.tenantId) {
