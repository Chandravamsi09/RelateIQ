/**
 * RelateIQ Domain Use Case: ProcessInvoiceItemUseCase
...
export interface ProcessInvoiceItemCommand {
  tenantId: string;
  actor: string;
...
export class ProcessInvoiceItemUseCase {
  ...
  public async execute(command: ProcessInvoiceItemCommand): Promise<{ success: boolean; result: any; auditEventId: string }> {
    if (!command.tenantId) {
