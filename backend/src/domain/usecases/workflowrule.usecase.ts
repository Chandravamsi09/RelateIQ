/**
 * RelateIQ Domain Use Case: ProcessWorkflowRuleUseCase
 * Implements business rule enforcement, event publication, and cross-aggregate orchestration.
 */

import { WorkflowRuleRepository } from '../repositories/workflowrule.repository';

export interface ProcessWorkflowRuleCommand {
  tenantId: string;
  actor: string;
...
export class ProcessWorkflowRuleUseCase {
  private repository: WorkflowRuleRepository;

  constructor() {
    this.repository = new WorkflowRuleRepository();
  }

  public async execute(command: ProcessWorkflowRuleCommand): Promise<{ success: boolean; result: any; auditEventId: string }> {
    if (!command.tenantId) {
