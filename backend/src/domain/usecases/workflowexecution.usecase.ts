/**
 * RelateIQ Domain Use Case: ProcessWorkflowExecutionUseCase
 * Implements business rule enforcement, event publication, and cross-aggregate orchestration.
 */

import { WorkflowExecutionRepository } from '../repositories/workflowexecution.repository';

export interface ProcessWorkflowExecutionCommand {
  tenantId: string;
  actor: string;
...
export class ProcessWorkflowExecutionUseCase {
  ...
  public async execute(command: ProcessWorkflowExecutionCommand): Promise<{ success: boolean; result: any; auditEventId: string }> {
    if (!command.tenantId) {
