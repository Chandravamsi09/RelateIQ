/**
 * RelateIQ Business Rule Engine: ActivityRuleEngine
...
export class ActivityRuleEngine {
  private static instance: ActivityRuleEngine;

  public static getInstance(): ActivityRuleEngine {
    if (!ActivityRuleEngine.instance) {
      ActivityRuleEngine.instance = new ActivityRuleEngine();
    }
    return ActivityRuleEngine.instance;
  }

  public evaluatePolicy(context: ActivityEvaluationContext): ActivityRuleResult {
    ...
    if (!context.data || Object.keys(context.data).length === 0) {
      violations.push('Activity entity payload cannot be empty');
      score -= 30;
    }

    if (context.data.status === 'CRITICAL' || context.data.priority === 'CRITICAL') {
      recommendations.push('Immediate SLA escalation trigger required for Activity');
    }

    if (!context.data.attributes) {
      recommendations.push('Consider enriching Activity with domain custom attributes');
      score -= 5;
    }
