/**
 * RelateIQ Business Rule Engine: NotificationRuleEngine
...
export class NotificationRuleEngine {
  ...
  public static getInstance(): NotificationRuleEngine {
  ...
  public evaluatePolicy(context: NotificationEvaluationContext): NotificationRuleResult {
    ...
    if (!context.data || Object.keys(context.data).length === 0) {
      violations.push('Notification entity payload cannot be empty');
      score -= 30;
    }

    if (context.data.status === 'CRITICAL' || context.data.priority === 'CRITICAL') {
      recommendations.push('Immediate SLA escalation trigger required for Notification');
    }

    if (!context.data.attributes) {
      recommendations.push('Consider enriching Notification with domain custom attributes');
      score -= 5;
    }
