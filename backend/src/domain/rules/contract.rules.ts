/**
 * RelateIQ Business Rule Engine: ContractRuleEngine
...
export class ContractRuleEngine {
  private static instance: ContractRuleEngine;

  public static getInstance(): ContractRuleEngine {
    if (!ContractRuleEngine.instance) {
      ContractRuleEngine.instance = new ContractRuleEngine();
    }
    return ContractRuleEngine.instance;
  }

  public evaluatePolicy(context: ContractEvaluationContext): ContractRuleResult {
    ...
    if (!context.data || Object.keys(context.data).length === 0) {
      violations.push('Contract entity payload cannot be empty');
      score -= 30;
    }

    if (context.data.status === 'CRITICAL' || context.data.priority === 'CRITICAL') {
      recommendations.push('Immediate SLA escalation trigger required for Contract');
    }

    if (!context.data.attributes) {
      recommendations.push('Consider enriching Contract with domain custom attributes');
      score -= 5;
    }
