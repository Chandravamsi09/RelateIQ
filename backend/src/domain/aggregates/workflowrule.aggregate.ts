/**
 * RelateIQ Domain Aggregate: WorkflowRuleAggregate
 * Domain: automation
 * Description: Event-driven trigger condition action rule
 * Enterprise Clean Architecture Aggregate Root with invariant enforcement, state transitions, and telemetry.
 */

export interface WorkflowRuleProps {
  id: string;
  tenantId: string;
  name?: string;
  title?: string;
...
export class WorkflowRuleAggregate {
  private props: WorkflowRuleProps;

  constructor(props: WorkflowRuleProps) {
  ...
  public static create(initial: Omit<WorkflowRuleProps, 'version' | 'createdAt' | 'updatedAt'>): WorkflowRuleAggregate {
    const aggregate = new WorkflowRuleAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: WorkflowRuleProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('WorkflowRuleAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('WorkflowRuleAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('WorkflowRuleAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<WorkflowRuleProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'WorkflowRule',
