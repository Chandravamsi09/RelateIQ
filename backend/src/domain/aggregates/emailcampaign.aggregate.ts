/**
 * RelateIQ Domain Aggregate: EmailCampaignAggregate
 * Domain: marketing
 * Description: Outbound marketing and nurture cadence
...
export class EmailCampaignAggregate {
  private props: EmailCampaignProps;

  constructor(props: EmailCampaignProps) {
  ...
  public static create(initial: Omit<EmailCampaignProps, 'version' | 'createdAt' | 'updatedAt'>): EmailCampaignAggregate {
    const aggregate = new EmailCampaignAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: EmailCampaignProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('EmailCampaignAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('EmailCampaignAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('EmailCampaignAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<EmailCampaignProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'EmailCampaign',
