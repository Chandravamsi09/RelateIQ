/**
 * RelateIQ Domain Aggregate: CallLogAggregate
 * Domain: omnichannel
 * Description: Inbound/outbound call transcript and duration
...
export class CallLogAggregate {
  private props: CallLogProps;
  ...
  public static create(initial: Omit<CallLogProps, 'version' | 'createdAt' | 'updatedAt'>): CallLogAggregate {
    const aggregate = new CallLogAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: CallLogProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('CallLogAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('CallLogAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('CallLogAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<CallLogProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'CallLog',
