/**
 * RelateIQ Domain Aggregate: TenantAggregate
 * Domain: identity
...
export class TenantAggregate {
  ...
  public static create(initial: Omit<TenantProps, 'version' | 'createdAt' | 'updatedAt'>): TenantAggregate {
  ...
  private validateInvariants(props: TenantProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('TenantAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('TenantAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('TenantAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<TenantProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'Tenant',
