/**
 * RelateIQ Domain Aggregate: ContractAggregate
 * Domain: billing
 * Description: Master services agreement and SLA commitment
...
export class ContractAggregate {
  ...
  public static create(initial: Omit<ContractProps, 'version' | 'createdAt' | 'updatedAt'>): ContractAggregate {
    const aggregate = new ContractAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: ContractProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('ContractAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('ContractAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('ContractAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<ContractProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'Contract',
