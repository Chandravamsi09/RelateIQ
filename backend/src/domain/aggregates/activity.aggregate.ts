/**
 * RelateIQ Domain Aggregate: ActivityAggregate
 * Domain: omnichannel
 * Description: Omnichannel activity timeline record
...
export class ActivityAggregate {
  ...
  public static create(initial: Omit<ActivityProps, 'version' | 'createdAt' | 'updatedAt'>): ActivityAggregate {
    const aggregate = new ActivityAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: ActivityProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('ActivityAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('ActivityAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('ActivityAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<ActivityProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'Activity',
