/**
 * RelateIQ Domain Aggregate: SalesQuotaAggregate
 * Domain: sales_ops
 * Description: Sales representative quota pacing
 * Enterprise Clean Architecture Aggregate Root with invariant enforcement, state transitions, and telemetry.
 */

export interface SalesQuotaProps {
  id: string;
  tenantId: string;
  name?: string;
  title?: string;
...
export class SalesQuotaAggregate {
  private props: SalesQuotaProps;

  constructor(props: SalesQuotaProps) {
  ...
  public static create(initial: Omit<SalesQuotaProps, 'version' | 'createdAt' | 'updatedAt'>): SalesQuotaAggregate {
    const aggregate = new SalesQuotaAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: SalesQuotaProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('SalesQuotaAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('SalesQuotaAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('SalesQuotaAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<SalesQuotaProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'SalesQuota',
