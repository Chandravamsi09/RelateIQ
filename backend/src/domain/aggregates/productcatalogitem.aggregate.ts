/**
 * RelateIQ Domain Aggregate: ProductCatalogItemAggregate
 * Domain: cpq
 * Description: CPQ product and pricing tiers
...
export class ProductCatalogItemAggregate {
  private props: ProductCatalogItemProps;

  constructor(props: ProductCatalogItemProps) {
  ...
  public static create(initial: Omit<ProductCatalogItemProps, 'version' | 'createdAt' | 'updatedAt'>): ProductCatalogItemAggregate {
    const aggregate = new ProductCatalogItemAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: ProductCatalogItemProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('ProductCatalogItemAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('ProductCatalogItemAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('ProductCatalogItemAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<ProductCatalogItemProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'ProductCatalogItem',
