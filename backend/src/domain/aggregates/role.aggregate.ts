/**
 * RelateIQ Domain Aggregate: RoleAggregate
 * Domain: identity
 * Description: Role-based access control definition
...
export class RoleAggregate {
  ...
  public static create(initial: Omit<RoleProps, 'version' | 'createdAt' | 'updatedAt'>): RoleAggregate {
  ...
  private validateInvariants(props: RoleProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('RoleAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('RoleAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('RoleAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<RoleProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'Role',
