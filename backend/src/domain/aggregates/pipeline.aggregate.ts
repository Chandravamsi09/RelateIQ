/**
 * RelateIQ Domain Aggregate: PipelineAggregate
 * Domain: pipeline
 * Description: Sales pipeline and process workflow
...
export class PipelineAggregate {
  ...
  public static create(initial: Omit<PipelineProps, 'version' | 'createdAt' | 'updatedAt'>): PipelineAggregate {
    const aggregate = new PipelineAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: PipelineProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('PipelineAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('PipelineAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('PipelineAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<PipelineProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'Pipeline',
