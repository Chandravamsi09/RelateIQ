/**
 * RelateIQ Domain Aggregate: AccountAggregate
 * Domain: crm
 * Description: B2B Client organization and Customer 360
...
export class AccountAggregate {
  ...
  public static create(initial: Omit<AccountProps, 'version' | 'createdAt' | 'updatedAt'>): AccountAggregate {
  ...
  private validateInvariants(props: AccountProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('AccountAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('AccountAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('AccountAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<AccountProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'Account',
