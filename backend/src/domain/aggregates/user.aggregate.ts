/**
 * RelateIQ Domain Aggregate: UserAggregate
 * Domain: identity
...
export class UserAggregate {
  ...
  public static create(initial: Omit<UserProps, 'version' | 'createdAt' | 'updatedAt'>): UserAggregate {
  ...
  private validateInvariants(props: UserProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('UserAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('UserAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('UserAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'User',
