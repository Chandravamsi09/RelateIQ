/**
 * RelateIQ Domain Aggregate: ContactAggregate
 * Domain: crm
 * Description: Client stakeholder and communication contact
...
export class ContactAggregate {
  ...
  public static create(initial: Omit<ContactProps, 'version' | 'createdAt' | 'updatedAt'>): ContactAggregate {
  ...
  private validateInvariants(props: ContactProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('ContactAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('ContactAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('ContactAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<ContactProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'Contact',
