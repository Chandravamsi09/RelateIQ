/**
 * RelateIQ Domain Aggregate: TicketAggregate
 * Domain: support
 * Description: Help desk support incident case
...
export class TicketAggregate {
  ...
  public static create(initial: Omit<TicketProps, 'version' | 'createdAt' | 'updatedAt'>): TicketAggregate {
  ...
  private validateInvariants(props: TicketProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('TicketAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('TicketAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('TicketAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<TicketProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'Ticket',
