/**
 * RelateIQ Domain Aggregate: QuoteLineItemAggregate
 * Domain: cpq
 * Description: Individual line item in pricing quote
...
export class QuoteLineItemAggregate {
  private props: QuoteLineItemProps;

  constructor(props: QuoteLineItemProps) {
  ...
  public static create(initial: Omit<QuoteLineItemProps, 'version' | 'createdAt' | 'updatedAt'>): QuoteLineItemAggregate {
    const aggregate = new QuoteLineItemAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: QuoteLineItemProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('QuoteLineItemAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('QuoteLineItemAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('QuoteLineItemAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<QuoteLineItemProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'QuoteLineItem',
