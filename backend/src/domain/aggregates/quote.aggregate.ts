/**
 * RelateIQ Domain Aggregate: QuoteAggregate
 * Domain: cpq
 * Description: Price quotation and discount matrix
...
export class QuoteAggregate {
  ...
  public static create(initial: Omit<QuoteProps, 'version' | 'createdAt' | 'updatedAt'>): QuoteAggregate {
    const aggregate = new QuoteAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: QuoteProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('QuoteAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('QuoteAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('QuoteAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<QuoteProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'Quote',
