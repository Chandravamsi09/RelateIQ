/**
 * RelateIQ Domain Aggregate: CustomerSatisfactionSurveyAggregate
 * Domain: support
 * Description: CSAT and NPS feedback questionnaire
...
export class CustomerSatisfactionSurveyAggregate {
  private props: CustomerSatisfactionSurveyProps;

  constructor(props: CustomerSatisfactionSurveyProps) {
  ...
  public static create(initial: Omit<CustomerSatisfactionSurveyProps, 'version' | 'createdAt' | 'updatedAt'>): CustomerSatisfactionSurveyAggregate {
    const aggregate = new CustomerSatisfactionSurveyAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: CustomerSatisfactionSurveyProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('CustomerSatisfactionSurveyAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('CustomerSatisfactionSurveyAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('CustomerSatisfactionSurveyAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<CustomerSatisfactionSurveyProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'CustomerSatisfactionSurvey',
