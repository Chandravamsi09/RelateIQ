/**
 * RelateIQ Domain Aggregate: LeadAggregate
 * Domain: pipeline
 * Description: Inbound or outbound sales prospect
...
export class LeadAggregate {
  ...
  public static create(initial: Omit<LeadProps, 'version' | 'createdAt' | 'updatedAt'>): LeadAggregate {
  ...
  private validateInvariants(props: LeadProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('LeadAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('LeadAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('LeadAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<LeadProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'Lead',
