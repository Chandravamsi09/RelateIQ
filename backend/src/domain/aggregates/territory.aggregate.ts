/**
 * RelateIQ Domain Aggregate: TerritoryAggregate
 * Domain: sales_ops
 * Description: Geographic and vertical sales territory
...
export class TerritoryAggregate {
  private props: TerritoryProps;
  ...
  public static create(initial: Omit<TerritoryProps, 'version' | 'createdAt' | 'updatedAt'>): TerritoryAggregate {
    const aggregate = new TerritoryAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: TerritoryProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('TerritoryAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('TerritoryAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('TerritoryAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<TerritoryProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'Territory',
