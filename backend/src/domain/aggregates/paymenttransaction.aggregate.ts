/**
 * RelateIQ Domain Aggregate: PaymentTransactionAggregate
 * Domain: billing
 * Description: Payment gateway capture transaction
...
export class PaymentTransactionAggregate {
  ...
  public static create(initial: Omit<PaymentTransactionProps, 'version' | 'createdAt' | 'updatedAt'>): PaymentTransactionAggregate {
    const aggregate = new PaymentTransactionAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: PaymentTransactionProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('PaymentTransactionAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('PaymentTransactionAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('PaymentTransactionAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<PaymentTransactionProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'PaymentTransaction',
