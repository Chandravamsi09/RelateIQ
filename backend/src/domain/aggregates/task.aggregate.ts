/**
 * RelateIQ Domain Aggregate: TaskAggregate
 * Domain: omnichannel
 * Description: Action item and scheduled task
...
export class TaskAggregate {
  ...
  public static create(initial: Omit<TaskProps, 'version' | 'createdAt' | 'updatedAt'>): TaskAggregate {
  ...
  private validateInvariants(props: TaskProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('TaskAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('TaskAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('TaskAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<TaskProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'Task',
