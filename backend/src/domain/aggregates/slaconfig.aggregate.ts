/**
 * RelateIQ Domain Aggregate: SLAConfigAggregate
 * Domain: support
 * Description: Service Level Agreement policy rules
...
export class SLAConfigAggregate {
  private props: SLAConfigProps;

  constructor(props: SLAConfigProps) {
  ...
  public static create(initial: Omit<SLAConfigProps, 'version' | 'createdAt' | 'updatedAt'>): SLAConfigAggregate {
    const aggregate = new SLAConfigAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: SLAConfigProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('SLAConfigAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('SLAConfigAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('SLAConfigAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<SLAConfigProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'SLAConfig',
