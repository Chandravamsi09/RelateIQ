/**
 * RelateIQ Domain Aggregate: PipelineStageAggregate
 * Domain: pipeline
 * Description: Kanban stage in sales pipeline
...
export class PipelineStageAggregate {
  ...
  public static create(initial: Omit<PipelineStageProps, 'version' | 'createdAt' | 'updatedAt'>): PipelineStageAggregate {
    const aggregate = new PipelineStageAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: PipelineStageProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('PipelineStageAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('PipelineStageAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('PipelineStageAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<PipelineStageProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'PipelineStage',
