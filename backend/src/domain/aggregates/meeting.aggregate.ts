/**
 * RelateIQ Domain Aggregate: MeetingAggregate
 * Domain: omnichannel
 * Description: Client calendar meeting and demo sync
...
export class MeetingAggregate {
  ...
  public static create(initial: Omit<MeetingProps, 'version' | 'createdAt' | 'updatedAt'>): MeetingAggregate {
    const aggregate = new MeetingAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: MeetingProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('MeetingAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('MeetingAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('MeetingAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<MeetingProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'Meeting',
