/**
 * RelateIQ Domain Aggregate: CustomFieldDefinitionAggregate
 * Domain: customization
 * Description: Dynamic schema custom attribute
...
export class CustomFieldDefinitionAggregate {
  private props: CustomFieldDefinitionProps;

  constructor(props: CustomFieldDefinitionProps) {
  ...
  public static create(initial: Omit<CustomFieldDefinitionProps, 'version' | 'createdAt' | 'updatedAt'>): CustomFieldDefinitionAggregate {
    const aggregate = new CustomFieldDefinitionAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: CustomFieldDefinitionProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('CustomFieldDefinitionAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('CustomFieldDefinitionAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('CustomFieldDefinitionAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<CustomFieldDefinitionProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'CustomFieldDefinition',
