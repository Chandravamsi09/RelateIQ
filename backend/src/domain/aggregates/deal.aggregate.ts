/**
 * RelateIQ Domain Aggregate: DealAggregate
 * Domain: pipeline
 * Description: Sales opportunity and revenue pacing
...
export class DealAggregate {
  ...
  public static create(initial: Omit<DealProps, 'version' | 'createdAt' | 'updatedAt'>): DealAggregate {
    const aggregate = new DealAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: DealProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('DealAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('DealAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('DealAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<DealProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'Deal',
