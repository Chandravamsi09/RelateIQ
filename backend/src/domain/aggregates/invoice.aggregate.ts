/**
 * RelateIQ Domain Aggregate: InvoiceAggregate
 * Domain: billing
 * Description: Commercial billing statement and line items
...
export class InvoiceAggregate {
  ...
  public static create(initial: Omit<InvoiceProps, 'version' | 'createdAt' | 'updatedAt'>): InvoiceAggregate {
  ...
  private validateInvariants(props: InvoiceProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('InvoiceAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('InvoiceAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('InvoiceAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<InvoiceProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'Invoice',
