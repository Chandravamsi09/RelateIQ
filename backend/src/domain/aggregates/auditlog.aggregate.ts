/**
 * RelateIQ Domain Aggregate: AuditLogAggregate
 * Domain: compliance
...
export class AuditLogAggregate {
  ...
  public static create(initial: Omit<AuditLogProps, 'version' | 'createdAt' | 'updatedAt'>): AuditLogAggregate {
    const aggregate = new AuditLogAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: AuditLogProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('AuditLogAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('AuditLogAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('AuditLogAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<AuditLogProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'AuditLog',
