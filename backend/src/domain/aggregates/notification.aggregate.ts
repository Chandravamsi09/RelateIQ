/**
 * RelateIQ Domain Aggregate: NotificationAggregate
 * Domain: collaboration
 * Description: In-app and push alert notification
...
export class NotificationAggregate {
  ...
  public static create(initial: Omit<NotificationProps, 'version' | 'createdAt' | 'updatedAt'>): NotificationAggregate {
  ...
  private validateInvariants(props: NotificationProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('NotificationAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('NotificationAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('NotificationAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'Notification',
