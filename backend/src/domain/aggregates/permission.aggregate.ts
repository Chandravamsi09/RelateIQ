/**
 * RelateIQ Domain Aggregate: PermissionAggregate
 * Domain: identity
 * Description: Granular authorization action privilege
...
export class PermissionAggregate {
  ...
  public static create(initial: Omit<PermissionProps, 'version' | 'createdAt' | 'updatedAt'>): PermissionAggregate {
  ...
  private validateInvariants(props: PermissionProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('PermissionAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('PermissionAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('PermissionAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<PermissionProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'Permission',
