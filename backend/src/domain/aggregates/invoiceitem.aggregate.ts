/**
 * RelateIQ Domain Aggregate: InvoiceItemAggregate
 * Domain: billing
 * Description: Individual line item on billing invoice
...
export class InvoiceItemAggregate {
  ...
  public static create(initial: Omit<InvoiceItemProps, 'version' | 'createdAt' | 'updatedAt'>): InvoiceItemAggregate {
    const aggregate = new InvoiceItemAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: InvoiceItemProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('InvoiceItemAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('InvoiceItemAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('InvoiceItemAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<InvoiceItemProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'InvoiceItem',
