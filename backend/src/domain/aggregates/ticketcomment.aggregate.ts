/**
 * RelateIQ Domain Aggregate: TicketCommentAggregate
 * Domain: support
 * Description: Discussion note on support ticket
...
export class TicketCommentAggregate {
  private props: TicketCommentProps;

  constructor(props: TicketCommentProps) {
  ...
  public static create(initial: Omit<TicketCommentProps, 'version' | 'createdAt' | 'updatedAt'>): TicketCommentAggregate {
    const aggregate = new TicketCommentAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: TicketCommentProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('TicketCommentAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('TicketCommentAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('TicketCommentAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<TicketCommentProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'TicketComment',
