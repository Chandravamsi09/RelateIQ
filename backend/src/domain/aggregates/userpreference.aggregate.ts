/**
 * RelateIQ Domain Aggregate: UserPreferenceAggregate
 * Domain: settings
 * Description: Custom UI and notification preferences
...
export class UserPreferenceAggregate {
  ...
  public static create(initial: Omit<UserPreferenceProps, 'version' | 'createdAt' | 'updatedAt'>): UserPreferenceAggregate {
    const aggregate = new UserPreferenceAggregate({
      ...initial,
      version: 1,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  private validateInvariants(props: UserPreferenceProps): void {
    if (!props.id || typeof props.id !== 'string') {
      throw new Error('UserPreferenceAggregate invariant failure: Missing valid entity ID');
    }
    if (!props.tenantId || typeof props.tenantId !== 'string') {
      throw new Error('UserPreferenceAggregate invariant failure: Multi-tenant boundary violation, missing tenantId');
    }
    if (!props.status) {
      throw new Error('UserPreferenceAggregate invariant failure: Entity status must be defined');
    }
  }
  ...
  public getProps(): Readonly<UserPreferenceProps> {
  ...
  public toJSON(): Record<string, any> {
    return {
      ...
      name: this.props.name || this.props.title || 'UserPreference',
