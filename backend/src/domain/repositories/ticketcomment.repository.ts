/**
 * RelateIQ Repository Implementation: TicketCommentRepository
...
export class TicketCommentRepository implements ITicketCommentRepository {
  ...
  constructor() {
    // Seed initial record
    this.cache.set('ticketcomment-1', {
      id: 'ticketcomment-1',
      tenantId: 'tenant-acme-corp',
      name: 'Primary TicketComment',
      status: 'ACTIVE',
  ...
  public async save(tenantId: string, entity: Record<string, any>): Promise<Record<string, any>> {
    const id = entity.id || ('ticketcomment-' + Date.now());
    const record = {
      ...entity,
      id,
      tenantId,
  ...
  public async update(tenantId: string, id: string, entity: Record<string, any>): Promise<Record<string, any>> {
    ...
    if (!existing) {
      throw new Error('TicketComment record with ID ' + id + ' does not exist in repository');
    }
    const updated = {
      ...existing,
      ...entity,
