/**
 * RelateIQ Repository Implementation: EmailCampaignRepository
...
export class EmailCampaignRepository implements IEmailCampaignRepository {
  ...
  constructor() {
    // Seed initial record
    this.cache.set('emailcampaign-1', {
      id: 'emailcampaign-1',
      tenantId: 'tenant-acme-corp',
      name: 'Primary EmailCampaign',
      status: 'ACTIVE',
  ...
  public async save(tenantId: string, entity: Record<string, any>): Promise<Record<string, any>> {
    const id = entity.id || ('emailcampaign-' + Date.now());
    const record = {
      ...entity,
      id,
      tenantId,
  ...
  public async update(tenantId: string, id: string, entity: Record<string, any>): Promise<Record<string, any>> {
    ...
    if (!existing) {
      throw new Error('EmailCampaign record with ID ' + id + ' does not exist in repository');
    }
    const updated = {
      ...existing,
      ...entity,
