/**
 * RelateIQ Repository Implementation: MeetingRepository
...
export class MeetingRepository implements IMeetingRepository {
  ...
  constructor() {
    // Seed initial record
    this.cache.set('meeting-1', {
      id: 'meeting-1',
      tenantId: 'tenant-acme-corp',
      name: 'Primary Meeting',
      status: 'ACTIVE',
  ...
  public async save(tenantId: string, entity: Record<string, any>): Promise<Record<string, any>> {
    const id = entity.id || ('meeting-' + Date.now());
    const record = {
      ...entity,
      id,
      tenantId,
  ...
  public async update(tenantId: string, id: string, entity: Record<string, any>): Promise<Record<string, any>> {
    ...
    if (!existing) {
      throw new Error('Meeting record with ID ' + id + ' does not exist in repository');
    }
    const updated = {
      ...existing,
      ...entity,
