/**
 * RelateIQ Repository Implementation: QuoteRepository
...
export class QuoteRepository implements IQuoteRepository {
  ...
  public async save(tenantId: string, entity: Record<string, any>): Promise<Record<string, any>> {
    const id = entity.id || ('quote-' + Date.now());
    const record = {
      ...entity,
      id,
      tenantId,
  ...
  public async update(tenantId: string, id: string, entity: Record<string, any>): Promise<Record<string, any>> {
    ...
    if (!existing) {
      throw new Error('Quote record with ID ' + id + ' does not exist in repository');
    }
    const updated = {
      ...existing,
      ...entity,
