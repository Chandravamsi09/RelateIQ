/**
 * RelateIQ Repository Implementation: WebhookEndpointRepository
...
export class WebhookEndpointRepository implements IWebhookEndpointRepository {
  ...
  constructor() {
    // Seed initial record
    this.cache.set('webhookendpoint-1', {
      id: 'webhookendpoint-1',
      tenantId: 'tenant-acme-corp',
      name: 'Primary WebhookEndpoint',
      status: 'ACTIVE',
  ...
  public async save(tenantId: string, entity: Record<string, any>): Promise<Record<string, any>> {
    const id = entity.id || ('webhookendpoint-' + Date.now());
    const record = {
      ...entity,
      id,
      tenantId,
  ...
  public async update(tenantId: string, id: string, entity: Record<string, any>): Promise<Record<string, any>> {
    ...
    if (!existing) {
      throw new Error('WebhookEndpoint record with ID ' + id + ' does not exist in repository');
    }
    const updated = {
      ...existing,
      ...entity,
