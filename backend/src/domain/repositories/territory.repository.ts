/**
 * RelateIQ Repository Implementation: TerritoryRepository
...
export class TerritoryRepository implements ITerritoryRepository {
  private cache: Map<string, Record<string, any>> = new Map();

  constructor() {
    // Seed initial record
    this.cache.set('territory-1', {
      id: 'territory-1',
      tenantId: 'tenant-acme-corp',
      name: 'Primary Territory',
      status: 'ACTIVE',
  ...
  public async save(tenantId: string, entity: Record<string, any>): Promise<Record<string, any>> {
    const id = entity.id || ('territory-' + Date.now());
    const record = {
      ...entity,
      id,
      tenantId,
  ...
  public async update(tenantId: string, id: string, entity: Record<string, any>): Promise<Record<string, any>> {
    ...
    if (!existing) {
      throw new Error('Territory record with ID ' + id + ' does not exist in repository');
    }
    const updated = {
      ...existing,
      ...entity,
