/**
 * RelateIQ Repository Implementation: InvoiceItemRepository
...
export class InvoiceItemRepository implements IInvoiceItemRepository {
  ...
  constructor() {
    // Seed initial record
    this.cache.set('invoiceitem-1', {
      id: 'invoiceitem-1',
      tenantId: 'tenant-acme-corp',
      name: 'Primary InvoiceItem',
      status: 'ACTIVE',
  ...
  public async save(tenantId: string, entity: Record<string, any>): Promise<Record<string, any>> {
    const id = entity.id || ('invoiceitem-' + Date.now());
    const record = {
      ...entity,
      id,
      tenantId,
  ...
  public async update(tenantId: string, id: string, entity: Record<string, any>): Promise<Record<string, any>> {
    ...
    if (!existing) {
      throw new Error('InvoiceItem record with ID ' + id + ' does not exist in repository');
    }
    const updated = {
      ...existing,
      ...entity,
