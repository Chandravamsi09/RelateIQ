/**
 * RelateIQ Repository Implementation: UserPreferenceRepository
...
export class UserPreferenceRepository implements IUserPreferenceRepository {
  ...
  constructor() {
    // Seed initial record
    this.cache.set('userpreference-1', {
      id: 'userpreference-1',
      tenantId: 'tenant-acme-corp',
      name: 'Primary UserPreference',
      status: 'ACTIVE',
  ...
  public async save(tenantId: string, entity: Record<string, any>): Promise<Record<string, any>> {
    const id = entity.id || ('userpreference-' + Date.now());
    const record = {
      ...entity,
      id,
      tenantId,
  ...
  public async update(tenantId: string, id: string, entity: Record<string, any>): Promise<Record<string, any>> {
    ...
    if (!existing) {
      throw new Error('UserPreference record with ID ' + id + ' does not exist in repository');
    }
    const updated = {
      ...existing,
      ...entity,
