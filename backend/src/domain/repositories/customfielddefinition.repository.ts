/**
 * RelateIQ Repository Implementation: CustomFieldDefinitionRepository
...
export class CustomFieldDefinitionRepository implements ICustomFieldDefinitionRepository {
  private cache: Map<string, Record<string, any>> = new Map();

  constructor() {
    // Seed initial record
    this.cache.set('customfielddefinition-1', {
      id: 'customfielddefinition-1',
      tenantId: 'tenant-acme-corp',
      name: 'Primary CustomFieldDefinition',
      status: 'ACTIVE',
  ...
  public async save(tenantId: string, entity: Record<string, any>): Promise<Record<string, any>> {
    const id = entity.id || ('customfielddefinition-' + Date.now());
    const record = {
      ...entity,
      id,
      tenantId,
  ...
  public async update(tenantId: string, id: string, entity: Record<string, any>): Promise<Record<string, any>> {
    ...
    if (!existing) {
      throw new Error('CustomFieldDefinition record with ID ' + id + ' does not exist in repository');
    }
    const updated = {
      ...existing,
      ...entity,
