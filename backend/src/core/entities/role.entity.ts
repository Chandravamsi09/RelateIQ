...
export class RoleDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('Role ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('Role Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('Role name cannot be blank');
    }
