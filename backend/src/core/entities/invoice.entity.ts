...
export class InvoiceDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('Invoice ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('Invoice Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('Invoice name cannot be blank');
    }
