...
export class ContactDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('Contact ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('Contact Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('Contact name cannot be blank');
    }
