...
export class EmailCampaignDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('EmailCampaign ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('EmailCampaign Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('EmailCampaign name cannot be blank');
    }
