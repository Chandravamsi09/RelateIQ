...
export class TicketDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('Ticket ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('Ticket Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('Ticket name cannot be blank');
    }
