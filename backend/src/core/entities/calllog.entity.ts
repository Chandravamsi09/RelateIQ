...
export class CallLogDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('CallLog ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('CallLog Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('CallLog name cannot be blank');
    }
