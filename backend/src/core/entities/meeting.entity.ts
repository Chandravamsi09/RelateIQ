...
export class MeetingDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('Meeting ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('Meeting Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('Meeting name cannot be blank');
    }
