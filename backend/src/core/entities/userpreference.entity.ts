...
export class UserPreferenceDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('UserPreference ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('UserPreference Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('UserPreference name cannot be blank');
    }
