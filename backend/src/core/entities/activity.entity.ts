...
export class ActivityDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('Activity ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('Activity Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('Activity name cannot be blank');
    }
