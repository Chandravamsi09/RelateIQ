...
export class TicketCommentDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('TicketComment ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('TicketComment Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('TicketComment name cannot be blank');
    }
