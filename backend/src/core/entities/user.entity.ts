...
export class UserDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('User ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('User Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('User name cannot be blank');
    }
