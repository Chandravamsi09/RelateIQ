...
export class PipelineDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('Pipeline ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('Pipeline Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('Pipeline name cannot be blank');
    }
