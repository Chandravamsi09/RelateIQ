...
/**
 * Domain Entity: CustomFieldDefinitionEntity
 * Encapsulates core business invariants, self-validation, mutation guards, and domain events.
 */
export class CustomFieldDefinitionDomainEntity {
  private id: UUID;
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('CustomFieldDefinition ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('CustomFieldDefinition Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('CustomFieldDefinition name cannot be blank');
    }
