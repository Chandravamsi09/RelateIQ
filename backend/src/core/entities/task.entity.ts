...
export class TaskDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('Task ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('Task Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('Task name cannot be blank');
    }
