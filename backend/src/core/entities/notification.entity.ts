...
export class NotificationDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('Notification ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('Notification Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('Notification name cannot be blank');
    }
