...
export class AuditLogDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('AuditLog ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('AuditLog Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('AuditLog name cannot be blank');
    }
