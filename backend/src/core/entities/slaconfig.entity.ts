...
export class SLAConfigDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('SLAConfig ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('SLAConfig Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('SLAConfig name cannot be blank');
    }
