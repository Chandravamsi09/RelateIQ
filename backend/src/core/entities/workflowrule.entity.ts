...
export class WorkflowRuleDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('WorkflowRule ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('WorkflowRule Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('WorkflowRule name cannot be blank');
    }
