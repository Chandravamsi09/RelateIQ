...
export class WebhookEndpointDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('WebhookEndpoint ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('WebhookEndpoint Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('WebhookEndpoint name cannot be blank');
    }
