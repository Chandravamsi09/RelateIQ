...
export class DealDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('Deal ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('Deal Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('Deal name cannot be blank');
    }
