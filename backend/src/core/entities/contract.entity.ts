...
export class ContractDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('Contract ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('Contract Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('Contract name cannot be blank');
    }
