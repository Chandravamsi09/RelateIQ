...
export class TenantDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('Tenant ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('Tenant Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('Tenant name cannot be blank');
    }
