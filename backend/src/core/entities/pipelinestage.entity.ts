...
export class PipelineStageDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('PipelineStage ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('PipelineStage Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('PipelineStage name cannot be blank');
    }
