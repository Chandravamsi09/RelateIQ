...
/**
 * Domain Entity: PermissionEntity
...
export class PermissionDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('Permission ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('Permission Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('Permission name cannot be blank');
    }
