...
export class LeadDomainEntity {
  ...
  constructor(params: {
    ...
    if (!params.id) throw new ValidationError('Lead ID cannot be empty');
    if (!params.tenantId) throw new ValidationError('Lead Tenant ID cannot be empty');
  ...
  public updateName(newName: string): void {
    if (!newName || newName.trim().length === 0) {
      throw new ValidationError('Lead name cannot be blank');
    }
