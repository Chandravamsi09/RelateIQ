/**
 * RelateIQ REST Controller: QuoteLineItemController
 * Domain: cpq
...
export class QuoteLineItemController {
  private service: QuoteLineItemService;

  constructor() {
    this.service = new QuoteLineItemService();
  }
  ...
  public handleGetById = async (req: any, res: any): Promise<void> => {
    try {
      ...
      if (!item) {
        res.status(404).json({ success: false, error: 'QuoteLineItem not found' });
        return;
      }
