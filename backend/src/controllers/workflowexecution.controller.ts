/**
 * RelateIQ REST Controller: WorkflowExecutionController
 * Domain: automation
...
export class WorkflowExecutionController {
  private service: WorkflowExecutionService;
  ...
  public handleGetById = async (req: any, res: any): Promise<void> => {
    try {
      ...
      if (!item) {
        res.status(404).json({ success: false, error: 'WorkflowExecution not found' });
        return;
      }
