/**
 * RelateIQ REST Controller: CustomerSatisfactionSurveyController
 * Domain: support
 * Express controller handling HTTP routes, query serialization, rate limiting, and RBAC permission checks.
 */

import { CustomerSatisfactionSurveyService } from '../services/customersatisfactionsurvey.service';

export class CustomerSatisfactionSurveyController {
  private service: CustomerSatisfactionSurveyService;

  constructor() {
    this.service = new CustomerSatisfactionSurveyService();
  }
  ...
  public handleGetById = async (req: any, res: any): Promise<void> => {
    try {
      ...
      if (!item) {
        res.status(404).json({ success: false, error: 'CustomerSatisfactionSurvey not found' });
        return;
      }
