/**
 * RelateIQ REST Controller: CustomFieldDefinitionController
 * Domain: customization
 * Express controller handling HTTP routes, query serialization, rate limiting, and RBAC permission checks.
 */

import { CustomFieldDefinitionService } from '../services/customfielddefinition.service';

export class CustomFieldDefinitionController {
  private service: CustomFieldDefinitionService;

  constructor() {
    this.service = new CustomFieldDefinitionService();
  }
  ...
  public handleGetById = async (req: any, res: any): Promise<void> => {
    try {
      ...
      if (!item) {
        res.status(404).json({ success: false, error: 'CustomFieldDefinition not found' });
        return;
      }
