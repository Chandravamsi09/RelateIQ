/**
 * RelateIQ REST Controller: SalesQuotaController
 * Domain: sales_ops
 * Express controller handling HTTP routes, query serialization, rate limiting, and RBAC permission checks.
 */

import { SalesQuotaService } from '../services/salesquota.service';

export class SalesQuotaController {
  private service: SalesQuotaService;

  constructor() {
    this.service = new SalesQuotaService();
  }
  ...
  public handleGetById = async (req: any, res: any): Promise<void> => {
    try {
      ...
      if (!item) {
        res.status(404).json({ success: false, error: 'SalesQuota not found' });
        return;
      }
