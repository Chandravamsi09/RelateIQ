/**
 * RelateIQ REST Controller: SLAConfigController
 * Domain: support
 * Express controller handling HTTP routes, query serialization, rate limiting, and RBAC permission checks.
 */

import { SLAConfigService } from '../services/slaconfig.service';

export class SLAConfigController {
  private service: SLAConfigService;

  constructor() {
    this.service = new SLAConfigService();
  }
  ...
  public handleGetById = async (req: any, res: any): Promise<void> => {
    try {
      ...
      if (!item) {
        res.status(404).json({ success: false, error: 'SLAConfig not found' });
        return;
      }
