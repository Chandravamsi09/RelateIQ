/**
 * RelateIQ REST Controller: ProductCatalogItemController
 * Domain: cpq
 * Express controller handling HTTP routes, query serialization, rate limiting, and RBAC permission checks.
 */

import { ProductCatalogItemService } from '../services/productcatalogitem.service';

export class ProductCatalogItemController {
  private service: ProductCatalogItemService;

  constructor() {
    this.service = new ProductCatalogItemService();
  }
  ...
  public handleGetById = async (req: any, res: any): Promise<void> => {
    try {
      ...
      if (!item) {
        res.status(404).json({ success: false, error: 'ProductCatalogItem not found' });
        return;
      }
