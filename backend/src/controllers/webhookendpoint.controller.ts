/**
 * RelateIQ REST Controller: WebhookEndpointController
 * Domain: integrations
...
export class WebhookEndpointController {
  private service: WebhookEndpointService;
  ...
  public handleGetById = async (req: any, res: any): Promise<void> => {
    try {
      ...
      if (!item) {
        res.status(404).json({ success: false, error: 'WebhookEndpoint not found' });
        return;
      }
