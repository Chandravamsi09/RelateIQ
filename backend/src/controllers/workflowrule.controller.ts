/**
 * RelateIQ REST Controller: WorkflowRuleController
 * Domain: automation
 * Express controller handling HTTP routes, query serialization, rate limiting, and RBAC permission checks.
 */

import { WorkflowRuleService } from '../services/workflowrule.service';

export class WorkflowRuleController {
  private service: WorkflowRuleService;
  ...
  public handleGetById = async (req: any, res: any): Promise<void> => {
    try {
      ...
      if (!item) {
        res.status(404).json({ success: false, error: 'WorkflowRule not found' });
        return;
      }
