/**
 * RelateIQ Domain Validator: PaymentTransactionValidationSchema
 * Enforces strong runtime schema validation, sanitization, and security filtering.
 */

export interface ValidatePaymentTransactionInput {
  name?: string;
  title?: string;
  status?: string;
...
export class PaymentTransactionValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidatePaymentTransactionInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidatePaymentTransactionInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidatePaymentTransactionInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
