/**
 * RelateIQ Domain Validator: PipelineStageValidationSchema
 * Enforces strong runtime schema validation, sanitization, and security filtering.
 */

export interface ValidatePipelineStageInput {
  name?: string;
  title?: string;
  status?: string;
...
export class PipelineStageValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidatePipelineStageInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidatePipelineStageInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidatePipelineStageInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
