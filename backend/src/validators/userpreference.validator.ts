/**
 * RelateIQ Domain Validator: UserPreferenceValidationSchema
 * Enforces strong runtime schema validation, sanitization, and security filtering.
 */

export interface ValidateUserPreferenceInput {
  name?: string;
  title?: string;
  status?: string;
...
export class UserPreferenceValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateUserPreferenceInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidateUserPreferenceInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateUserPreferenceInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
