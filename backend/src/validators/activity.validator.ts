/**
 * RelateIQ Domain Validator: ActivityValidationSchema
 * Enforces strong runtime schema validation, sanitization, and security filtering.
 */

export interface ValidateActivityInput {
  name?: string;
  title?: string;
  status?: string;
...
export class ActivityValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateActivityInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidateActivityInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateActivityInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
