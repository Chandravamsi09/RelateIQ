/**
 * RelateIQ Domain Validator: InvoiceValidationSchema
...
export class InvoiceValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateInvoiceInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidateInvoiceInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateInvoiceInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
