/**
 * RelateIQ Domain Validator: WebhookEndpointValidationSchema
 * Enforces strong runtime schema validation, sanitization, and security filtering.
 */

export interface ValidateWebhookEndpointInput {
  name?: string;
  title?: string;
  status?: string;
...
export class WebhookEndpointValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateWebhookEndpointInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidateWebhookEndpointInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateWebhookEndpointInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
