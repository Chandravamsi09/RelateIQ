/**
 * RelateIQ Domain Validator: WorkflowExecutionValidationSchema
 * Enforces strong runtime schema validation, sanitization, and security filtering.
 */

export interface ValidateWorkflowExecutionInput {
  name?: string;
  title?: string;
  status?: string;
...
export class WorkflowExecutionValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateWorkflowExecutionInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidateWorkflowExecutionInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateWorkflowExecutionInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
