/**
 * RelateIQ Domain Validator: TerritoryValidationSchema
 * Enforces strong runtime schema validation, sanitization, and security filtering.
 */

export interface ValidateTerritoryInput {
  name?: string;
  title?: string;
  status?: string;
...
export class TerritoryValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateTerritoryInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidateTerritoryInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateTerritoryInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
