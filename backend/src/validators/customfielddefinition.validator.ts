/**
 * RelateIQ Domain Validator: CustomFieldDefinitionValidationSchema
 * Enforces strong runtime schema validation, sanitization, and security filtering.
 */

export interface ValidateCustomFieldDefinitionInput {
  name?: string;
  title?: string;
  status?: string;
...
export class CustomFieldDefinitionValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateCustomFieldDefinitionInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidateCustomFieldDefinitionInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateCustomFieldDefinitionInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
