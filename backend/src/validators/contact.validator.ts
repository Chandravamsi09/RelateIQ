/**
 * RelateIQ Domain Validator: ContactValidationSchema
 * Enforces strong runtime schema validation, sanitization, and security filtering.
 */

export interface ValidateContactInput {
  name?: string;
  title?: string;
  status?: string;
...
export class ContactValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateContactInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidateContactInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateContactInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
