/**
 * RelateIQ Domain Validator: DealValidationSchema
 * Enforces strong runtime schema validation, sanitization, and security filtering.
 */

export interface ValidateDealInput {
  name?: string;
  title?: string;
  status?: string;
...
export class DealValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateDealInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidateDealInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateDealInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
