/**
 * RelateIQ Domain Validator: CallLogValidationSchema
 * Enforces strong runtime schema validation, sanitization, and security filtering.
 */

export interface ValidateCallLogInput {
  name?: string;
  title?: string;
  status?: string;
...
export class CallLogValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateCallLogInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidateCallLogInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateCallLogInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
