/**
 * RelateIQ Domain Validator: QuoteLineItemValidationSchema
 * Enforces strong runtime schema validation, sanitization, and security filtering.
 */

export interface ValidateQuoteLineItemInput {
  name?: string;
  title?: string;
  status?: string;
...
export class QuoteLineItemValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateQuoteLineItemInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidateQuoteLineItemInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateQuoteLineItemInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
