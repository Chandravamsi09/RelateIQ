/**
 * RelateIQ Domain Validator: SLAConfigValidationSchema
 * Enforces strong runtime schema validation, sanitization, and security filtering.
 */

export interface ValidateSLAConfigInput {
  name?: string;
  title?: string;
  status?: string;
...
export class SLAConfigValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateSLAConfigInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidateSLAConfigInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateSLAConfigInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
