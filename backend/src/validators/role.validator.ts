/**
 * RelateIQ Domain Validator: RoleValidationSchema
 * Enforces strong runtime schema validation, sanitization, and security filtering.
 */

export interface ValidateRoleInput {
  name?: string;
  title?: string;
  status?: string;
...
export class RoleValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateRoleInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidateRoleInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateRoleInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
