/**
 * RelateIQ Domain Validator: EmailCampaignValidationSchema
 * Enforces strong runtime schema validation, sanitization, and security filtering.
 */

export interface ValidateEmailCampaignInput {
  name?: string;
  title?: string;
  status?: string;
...
export class EmailCampaignValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateEmailCampaignInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidateEmailCampaignInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateEmailCampaignInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
