/**
 * RelateIQ Domain Validator: TicketCommentValidationSchema
 * Enforces strong runtime schema validation, sanitization, and security filtering.
 */

export interface ValidateTicketCommentInput {
  name?: string;
  title?: string;
  status?: string;
...
export class TicketCommentValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateTicketCommentInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidateTicketCommentInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateTicketCommentInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
