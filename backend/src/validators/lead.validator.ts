/**
 * RelateIQ Domain Validator: LeadValidationSchema
...
export class LeadValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateLeadInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidateLeadInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateLeadInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
