/**
 * RelateIQ Domain Validator: MeetingValidationSchema
...
export class MeetingValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateMeetingInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidateMeetingInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateMeetingInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
