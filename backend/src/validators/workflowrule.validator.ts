/**
 * RelateIQ Domain Validator: WorkflowRuleValidationSchema
 * Enforces strong runtime schema validation, sanitization, and security filtering.
 */

export interface ValidateWorkflowRuleInput {
  name?: string;
  title?: string;
  status?: string;
...
export class WorkflowRuleValidator {
  public static validateCreate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateWorkflowRuleInput } {
    const errors: string[] = [];
    ...
    const sanitized: ValidateWorkflowRuleInput = {
  ...
  public static validateUpdate(input: unknown): { isValid: boolean; errors: string[]; sanitized?: ValidateWorkflowRuleInput } {
    const errors: string[] = [];
    if (!input || typeof input !== 'object') {
