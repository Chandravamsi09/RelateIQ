...
export interface ProductCatalogItemDetailProps {
  id: string;
  tenantId: string;
  name?: string;
  status: string;
...
export const ProductCatalogItemDossierView: React.FC<ProductCatalogItemDetailProps> = ({ id, name, status, onBack }) => {
    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="flex items-center justify-between pb-4 border-b border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-100">{name || 'ProductCatalogItem Details'}</h2>
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-2 bg-slate-950 border border-slate-800 rounded-2xl p-6">
            <h3 className="text-sm font-semibold text-slate-200 mb-4">ProductCatalogItem Metadata & Attributes</h3>
            <div className="space-y-3 text-xs text-slate-300">
              <div className="flex justify-between p-3 rounded-xl bg-slate-900 border border-slate-800">
                ...
                <span className="font-semibold text-slate-200">CPQ</span>
              </div>
              <div className="flex justify-between p-3 rounded-xl bg-slate-900 border border-slate-800">
