...
export interface EmailCampaignFormWizardProps {
  tenantId?: string;
...
export const EmailCampaignFormWizard: React.FC<EmailCampaignFormWizardProps> = ({ tenantId = 'tenant-acme-corp', initialValues = {}, onSubmit, onCancel }) => {
  ...
  return (
    <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
      <div className="flex justify-between items-center pb-3 border-b border-slate-800">
        <h3 className="text-base font-bold text-slate-100">EmailCampaign Configuration Wizard</h3>
        <span className="text-xs px-2.5 py-0.5 rounded bg-blue-500/10 text-blue-400 font-mono">SOC2 Secure</span>
      </div>

      <form onSubmit={handleSubmit} className="space-y-3 text-xs">
        <div>
          <label className="block font-semibold text-slate-400 mb-1">EmailCampaign Identifier / Title *</label>
          <input
            type="text"
            required
            value={formData.name}
            onChange={e => handleChange('name', e.target.value)}
            placeholder="Enter EmailCampaign name..."
