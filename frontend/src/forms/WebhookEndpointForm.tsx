...
/**
 * Enterprise Form Component: WebhookEndpointForm
 * Handles field validation, multi-step validation rules, state binding, and submission lifecycle.
 */
export const WebhookEndpointForm: React.FC<WebhookEndpointFormProps> = ({
  initialValues = {},
  onSubmit,
  onCancel,
  isLoading = false
    ...
    <Card className="p-6 bg-slate-950 border-slate-800 max-w-2xl mx-auto shadow-2xl">
      <div className="mb-6 pb-4 border-b border-slate-800">
        <h3 className="text-xl font-bold text-slate-100">
          {initialValues?.id ? 'Edit ' + 'WebhookEndpoint' : 'Create New ' + 'WebhookEndpoint'}
        </h3>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
          ...
          <Button variant="primary" type="submit" disabled={isLoading}>
            {isLoading ? 'Saving...' : 'Save ' + 'WebhookEndpoint'}
          </Button>
        </div>
      </form>
    </Card>
