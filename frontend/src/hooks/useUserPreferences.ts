 ...
 * Provides reactive querying, mutations, caching, and WebSocket subscriptions for UserPreference entities.
 */
export function useUserPreferences(options: UseUserPreferencesOptions = {}) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/userpreferences`);
  ...
  const updateItem = async (id: string, updates: any) => {
    const res = await fetch(`/api/v1/userpreferences/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  const deleteItem = async (id: string) => {
    const res = await fetch(`/api/v1/userpreferences/${id}`, { method: 'DELETE' });
    const result = await res.json();
    if (result.success) {
