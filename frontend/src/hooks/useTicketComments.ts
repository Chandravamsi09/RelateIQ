...
/**
 * Custom React Hook: useTicketComments
 * Provides reactive querying, mutations, caching, and WebSocket subscriptions for TicketComment entities.
 */
export function useTicketComments(options: UseTicketCommentsOptions = {}) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/ticketcomments`);
  ...
  const updateItem = async (id: string, updates: any) => {
    const res = await fetch(`/api/v1/ticketcomments/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  const deleteItem = async (id: string) => {
    const res = await fetch(`/api/v1/ticketcomments/${id}`, { method: 'DELETE' });
    const result = await res.json();
    if (result.success) {
