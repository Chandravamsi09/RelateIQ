 ...
 * Provides reactive querying, mutations, caching, and WebSocket subscriptions for Permission entities.
 */
export function usePermissions(options: UsePermissionsOptions = {}) {
  ...
  const deleteItem = async (id: string) => {
    const res = await fetch(`/api/v1/permissions/${id}`, { method: 'DELETE' });
    const result = await res.json();
    if (result.success) {
