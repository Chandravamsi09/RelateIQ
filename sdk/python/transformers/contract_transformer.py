"""
RelateIQ Enterprise Python SDK - Contract Data Transformer & Pipeline Serializer
...
class ContractTransformer:
    """Transforms raw API responses and database dictionaries for Contract."""
    
    @staticmethod
    def transform_to_model_dict(raw: Dict[str, Any]) -> Dict[str, Any]:
        """Sanitizes and normalizes raw dictionary for Contract."""
        if not raw:
            return {}
        return {
            "id": str(raw.get("id", "")),
            "tenant_id": str(raw.get("tenantId", "tenant-acme-corp")),
            "name": raw.get("name") or raw.get("title") or "Contract",
    ...
    def batch_transform(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Batch transforms a collection of Contract items."""
        return [ContractTransformer.transform_to_model_dict(item) for item in items if item]
